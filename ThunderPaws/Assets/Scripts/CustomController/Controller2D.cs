...
public class Controller2D : RaycastController {
    ...
    void HorizontalCollisions(ref Vector2 moveAmount) {
        ...
        for (int i = 0; i < horizontalRayCount; ++i) {//no way this matters
            ...
            if (hit) {
                ...
                if(i == 0 && slopeAngle <= maxSlopeAngle) {//bottom most ray
    ...
    void DescendSlope(ref Vector2 moveAmount) {
        //handle slopes that exceed the max slope angle
        RaycastHit2D maxSlopeHitLeft = Physics2D.Raycast(raycastOrigins.bottomLeft, Vector2.down, Mathf.Abs(moveAmount.y) + skinWidth, collisionMask);
        RaycastHit2D maxSlopeHitRight = Physics2D.Raycast(raycastOrigins.bottomRight, Vector2.down, Mathf.Abs(moveAmount.y) + skinWidth, collisionMask);
        //xor operator - don't slide down unless one corner is not colliding which indicates we're ON the slope, not just above it
        ...
        if (!collisions.slidingDownMaxSlope) { 
            ...
            if (hit) {
                //handle slopes that are less than the max slope angle
                float slopeAngle = Vector2.Angle(hit.normal, Vector2.up);
    ...
    void SlideDownMaxSlope(RaycastHit2D hit, ref Vector2 moveAmount) {
        if (hit) {
            //figure out angle of slope
            float slopeAngle = Vector2.Angle(hit.normal, Vector2.up);
            if(slopeAngle > maxSlopeAngle) {
                //distance to move down the slope
